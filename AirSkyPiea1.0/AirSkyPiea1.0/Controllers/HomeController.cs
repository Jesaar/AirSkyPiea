...
namespace AirSkyPiea1._0.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index(string sortOrder, string currentFilter, string searchString, int? pageNumber)
        {
            ...
            if (!string.IsNullOrEmpty(searchString))
            {
                query = query.Where(p => (p.Name.ToLower().Contains(searchString.ToLower()) ||
                                          p.DestinationCategories.Any(pc => pc.Category.Name.ToLower().Contains(searchString.ToLower()))) &&
                                          p.Stock > 0);
            }
            else
            {
            ...
            {
                Destinations = await PaginatedList<Destination>.CreateAsync(query, pageNumber ?? 1, pageSize),
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            foreach (DestinationCategory category in destination.DestinationCategories)
            {
                categories += $"{category.Category.Name}, ";
            }
            categories = categories.Substring(0, categories.Length - 2);

            AddDestinationToCartViewModel model = new()
            {
        ...
        public async Task<IActionResult> Details(AddDestinationToCartViewModel model)
        {
            if (!User.Identity.IsAuthenticated)
            {
                return RedirectToAction("Login", "Account");
        ...
        public async Task<IActionResult> ShowCart()
        {
            ...
            List<TemporalSale> temporalSales = await _context.TemporalSales
                ...
                .ThenInclude(p => p.DestinationImages)
                .Where(ts => ts.User.Id == user.Id)
                .ToListAsync();
